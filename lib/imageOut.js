...
/** 右下角半透明底 + 白字；用 text-anchor=end，避免中文宽度估算偏出走出画面 */
function watermarkSvg(width, height) {
  const w = Math.max(1, width || 1024);
  const h = Math.max(1, height || 1024);
  const fontSize = Math.max(18, Math.round(Math.min(w, h) * 0.038));
  const pad = Math.max(14, Math.round(fontSize * 0.85));
  const boxH = Math.round(fontSize * 1.7);
  const boxW = Math.round(fontSize * WATERMARK_TEXT.length * 1.05 + 24);
  const x = w - pad;
  const y = h - pad;
  const bx = Math.max(0, x - boxW);
  const by = Math.max(0, y - boxH);
  // 多字体回退：镜像内装 Noto/WenQuanYi，本机开发也尽量有字
  const font =
    "Noto Sans CJK SC, WenQuanYi Zen Hei, PingFang SC, Microsoft YaHei, sans-serif";
  return Buffer.from(
    `<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
      <rect x="${bx}" y="${by}" width="${boxW}" height="${boxH}" rx="8" fill="rgba(0,0,0,0.45)"/>
      <text x="${x}" y="${y - Math.round(fontSize * 0.35)}" text-anchor="end"
        font-size="${fontSize}" font-family="${font}" font-weight="600"
        fill="rgba(255,255,255,0.96)">${WATERMARK_TEXT}</text>
    </svg>`
  );
}
