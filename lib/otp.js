 ...
 * 配置：
 *   SMTP_* — QQ 邮箱发信
 *   TENCENT_SECRET_ID / TENCENT_SECRET_KEY / SMS_SDK_APP_ID / SMS_SIGN / SMS_TEMPLATE_ID — 腾讯云短信
 *   OTP_SMS_URL — 可选自定义短信网关
 *   OTP_DEV=1 — 调试时响应带回验证码
 */
const fs = require("fs");
...
const tencentSms = require("./tencentSms");
...
async function sendSms(phone, code) {
  // 1) 腾讯云短信（推荐）
  if (tencentSms.isConfigured()) {
    await tencentSms.sendTencentSms(phone, code);
    return;
  }
  // 2) 自定义网关
  ...
  if (!url) {
    const err = new Error(
      "未配置短信：请设置腾讯云 SMS_SDK_APP_ID/SMS_SIGN/SMS_TEMPLATE_ID 与密钥，或 OTP_SMS_URL"
    );
    err.code = "NO_SMS";
    throw err;
  }
...
async function sendCode(accountRaw, ip) {
  ...
  if (!detected) {
    const err = new Error("请输入 +86 手机号或 QQ 邮箱（@qq.com）");
