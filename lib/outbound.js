 ...
 * 同一账号可走 backbone：p.webshare.io:80 / 1080 / 3128
 */
function deriveBackboneProxies(directUrls) {
  const seenCred = new Set();
  const out = [];
  const ports = [80, 1080, 3128];
...
function friendlyOutboundError(err) {
  ...
  if (/fetch failed|ECONN|ETIMEDOUT|UND_ERR|socket/i.test(detail)) {
    return `出站代理连不上上游（已试 ${Math.min(MAX_TRIES, list.length)} 条，mode=${PROXY_MODE}）。${detail.slice(0, 100)}。若持续 ECONNRESET：确认已发布最新版（默认走 p.webshare.io:80），并可选把出口 IP 加进 Webshare 白名单`;
  }
