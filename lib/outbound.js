...
function friendlyOutboundError(err) {
  ...
  if (/fetch failed|ECONN|ETIMEDOUT|UND_ERR|socket|Timeout/i.test(detail)) {
    return `出站代理连不上 Webshare（已试 ${Math.min(MAX_TRIES, list.length)} 条，mode=${PROXY_MODE}）。${detail.slice(0, 90)}。若已改用 VPS 中转，请把 DAIDAI_IMAGE_BASE_URL 设为 VPS 地址并重新发布（勿再经代理池）`;
  }
  return detail.slice(0, 200);
}

/** 只有直连官方 OpenAI 才需要代理池；VPS/自建中转必须直连，否则会套 Webshare 导致超时 504 */
function shouldUseProxyPool(targetUrl) {
  if (process.env.DAIDAI_OUTBOUND_PROXY === "0") return false;
  if (PROXY_MODE === "off" || PROXY_MODE === "none") return false;
  try {
...
async function outboundFetch(url, options = {}) {
  const useProxy = shouldUseProxyPool(url) && ensureLoaded().length > 0;
  if (!useProxy) {
    try {
